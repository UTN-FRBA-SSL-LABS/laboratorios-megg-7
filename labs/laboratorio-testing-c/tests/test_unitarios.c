...
void test_carrito_lleno(void) {
    printf("\n[carrito lleno]\n");
    Carrito c;
    carrito_init(&c); /*inicializar carrito*/
    Producto p = {"Leche", 350, 1};
    for (int i = 0; i < MAX_ITEMS; i++) {
        ASSERT_IGUAL(1, carrito_agregar(&c, p));  /* agregar hasta un maximo */
    }
    ASSERT_IGUAL(0, carrito_agregar(&c, p));  /* intentar agregar uno mas */
    ASSERT_IGUAL(MAX_ITEMS, carrito_contar(&c));  /* verificar cantidad */
}
